...
We recommend using [our GitHub Action](https://github.com/adits31/golangci-lint-action) for running `golangci-lint` in CI for GitHub projects.
It's [fast and uses smart caching](https://github.com/adits31/golangci-lint-action#performance) inside
...
It's highly recommended to install a specific version of golangci-lint available on the [releases page](https://github.com/adits31/golangci-lint/releases).
...
and is constantly being improved. For any problems with `golangci-lint`, check out recent [GitHub issues](https://github.com/adits31/golangci-lint/issues) and update if needed.
...
2. [`go.mod`](https://github.com/adits31/golangci-lint/blob/master/go.mod) replacement directive doesn't apply. It means a user will be using patched version of `golangci-lint` if we use such replacements.
...
```sh
go get github.com/adits31/golangci-lint/cmd/golangci-lint@{.LatestVersion}
```
...
For example, in our [GitHub Action](https://github.com/adits31/golangci-lint-action) we require users to explicitly set the minor version of `golangci-lint`
